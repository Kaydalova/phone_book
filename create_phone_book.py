# Дополнительный модуль для создания и заполнения телефонной книги тестовыми данными.
# В качестве фамилии, имени, отчества и названия организации берутся случайные слова
# из списка "Новый частотный словарь русской лексики" О. Н. Ляшевская, С. А. Шаров.
...
def generate_contact(common_words, index) -> str:
    """
    Функция генерирует случайные контактные данные для записи в телефонную книгу.
    """
    ...
    return f"{index};{last_name};{first_name};{patronymic};ООО {organization};{work_phone};{mobile_phone}\n"
...
def main():
    ...
    new_contacts = [generate_contact(common_words, i+1) for i in range(TEST_DATA_SIZE)]
